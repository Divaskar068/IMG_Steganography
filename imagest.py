...
key = np.random.randint(0, 256, dtype=np.uint8)
...
def add_noise():
    global img, key, noisy_image
    image = cv2.imread(filename, 0)
    noisy_image = np.bitwise_xor(image, key)
    save_image(noisy_image, "enc_image.png")
    messagebox.showinfo("SAVED")

def remove_noise():
    global noisy_image, key, restored_image, filename
    noisy_image = cv2.imread(filename, 0)
    restored_image = np.bitwise_xor(noisy_image, key)
    save_image(restored_image, "rwwestored_image.png")
...
Button(frame3, text="Open Image", width=5, height=2, font="arial 11 bold", command=showimage).place(x=20, y=30)
Button(frame3, text="Add Noise", width=5, height=2, font="arial 11 bold", command=add_noise).place(x=91, y=30)
Button(frame3, text="Remove Noise", width=5, height=2, font="arial 11 bold", command=remove_noise).place(x=164, y=30)
Button(frame3, text="Save Image", width=5, height=2, font="arial 11 bold", command=save).place(x=236, y=30)
